...
public class SupVil {
    ...
    public static void main(String[] st) throws NumberFormatException, IOException {
        ...
        while (noOfTestCases-- > 0) {
            int noOfFighters = Integer.parseInt(br.readLine().trim());
            int vilian = 0;
            int hero = 0;
            boolean fightEnd = false;
            String[] fighters = new String[noOfFighters];
            for (int i = 0; i < noOfFighters; i++) {
                fighters[i] = br.readLine().trim();
            }
            for (int i = 0; i < noOfFighters; i++) {
                if (fighters[i].endsWith("man") || fighters[i].endsWith("woman")) {
                    hero++;
                } else {
                    vilian++;
                }
